...
let rec make_move player field =
  ...
  | Field.Coordinate_out_of_bounds -> print_endline "Neveljavne koordinate";
	make_move player field
  | Field.Nonempty_value_at_coordinates -> print_endline "Zasedeno polje";
	make_move player field
  | Failure msg when msg = "int_of_string" -> print_endline "Vnesi celo število";
    make_move player field
