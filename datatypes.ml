...
let rec strongest_wizard (wizards : wizard list) : wizard option =
	match wizards with 
	| [] -> None
	| hd::tl -> let strongest_tl = strongest_wizard tl in 
		(match strongest_tl with 
		| None -> Some hd
		| Some w -> if hd.ability > w.ability 
		then Some hd else Some w)
...
let rec max_list (xs : 'a list) (max: 'a -> 'a -> 'a) : 'a option =
	match xs with
	| [] -> None
	| hd::tl -> match max_list tl max with
		| None -> Some hd
...
(* Write a function that calculates how much damages a spell causes to a
   ...
   Hint: float_of_int
*)
let damage_to_wiz spell wiz =
	int_of_float ((float_of_int (mana_of_spell spell)) *. damage_coefficient(vulnerable wiz spell))
