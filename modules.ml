...
module Nat_int : NAT = struct
  ...
  let one   = 1
  let plus  = (+)
  let minus x y = max 0 (x - y)
  let multiply x y = x * y
  let to_int t = t
  let from_int t = max t 0
end
...
module Nat_Peano = struct
  ...
  let rec plus x y = 
	...
	| (Zero, Succ(t)) 
	| (Succ(t), Zero) -> Succ(t)
	| (Succ(t), Succ(v)) -> Succ(Succ(plus t v))
  ...
  let rec multiply x y = 
	...
	| (_, Zero) -> Zero
	| (Succ(t), Succ(Zero))
	| (Succ(Zero), Succ(t)) -> Succ(t)
	| (Succ(t), Succ(v)) -> plus (plus (multiply t v) (Succ(Zero))) (plus t v)
end
