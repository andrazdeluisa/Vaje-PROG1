...
let rec insert y xs =
	match xs with
	| [] -> y :: []
	| hd :: tl -> if y < hd then y :: xs else hd :: insert y tl
	
(* How to prevent shadowing of variables in the recursion

let rec insert y xs =
	match xs with
	| [] -> y :: []
	| (x::xs) as xs_old -> if y < x then y :: xs_old else x :: insert y xs 
*)
...
let ins_sort l = 
	let rec ins_sort_aux acc l = 
		match l with
		| [] -> acc
		| x :: xs -> let acc = insert x acc 
		in ins_sort_aux acc xs
		in ins_sort_aux [] l
...
let rec min_and_rest l = 
	...
	| x :: xs -> 
		...
		in
		let rec minimum min_so_far l = failwith ""
		in let z = minimum x xs in 
